...
//		static private const 	MOUSE_LOOK_CHANGE_RATE:int 	= 10000;
    ...
    static public function getAnimationClass():String {
...
//						return 0;
    ...
    override public function update($context:Context3D, $elapsedTimeMS:int):void {
        // What should happen if we are riding something?
    ...
    override protected function handleMouseMovement($elapsedTimeMS:int):void {
        if (Globals.active
            ...
            if (MouseKeyboardHandler.isLeftMouseDown) {
                ...
                if (onSolidGround) {
                    instanceInfo.rotationSetComp(currentBodyRot.x, newY, 0);
                } else {
//                    instanceInfo.rotationSetComp(currentBodyRot.x + dx, newY, 0);
                    //Log.out( "Avatar.handleMouseMovement - TEMPORARILY disable rotation around X axis when not on solid ground", Log.WARN );
                    instanceInfo.rotationSetComp(currentBodyRot.x, newY, 0);
                }
        ...
        function setCameraAndHead($x:Number, $y:Number, $z:Number):void {
