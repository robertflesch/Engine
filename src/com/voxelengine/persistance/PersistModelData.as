...
public class PersistModelData
{
	...
	static public function addEvents():void {
		...
		ModelDataPersistanceEvent.addListener( PersistanceEvent.LOAD_REQUEST, 		load );
		//ModelMetadataPersistanceEvent.addListener( PersistanceEvent.SAVE_REQUEST, 		save );
	}

	static private function load( $mmpe:ModelDataPersistanceEvent ):void {
		...
		Log.out( "PersistRegion.loadRegion - guid: " + $mmpe.guid, Log.DEBUG ); 
		...
		function succeedLoad( dbo:DatabaseObject ):void {
			Log.out( "PersistRegion.load.succeed - loaded guid: " + $mmpe.guid ); 
			if ( dbo )
				ModelDataPersistanceEvent.dispatch( new ModelDataPersistanceEvent( PersistanceEvent.LOAD_SUCCEED, $mmpe.guid, dbo ) );
			else	
				ModelDataPersistanceEvent.dispatch( new ModelDataPersistanceEvent( PersistanceEvent.LOAD_FAILED, $mmpe.guid ) );
				
		}
		
		function failLoad(e:PlayerIOError):void 
		{  
			Log.out( "PersistRegion.load.fail - guid: " + $mmpe.guid + " error: " + e.message, Log.ERROR, e ); 
			ModelDataPersistanceEvent.dispatch( new ModelDataPersistanceEvent( PersistanceEvent.LOAD_FAILED, $mmpe.guid ) );
		}
		
		function errorNoClientLoad($pe:PlayerIOPersistanceEvent):void {
			Log.out( "PersistRegion.load.errorNoClient - guid: " + $mmpe.guid + "  error data: NOT CONNECTED TO THE INTERNET", Log.ERROR ) 
			ModelDataPersistanceEvent.dispatch( new ModelDataPersistanceEvent( PersistanceEvent.LOAD_FAILED, $mmpe.guid ) );
		}		
		
		function errorNoDBLoad($pe:PlayerIOPersistanceEvent):void {
			Log.out( "PersistRegion.load.errorNoDB - guid: " + $mmpe.guid + "  error data: DATABASE NOT FOUND", Log.ERROR ) 
			ModelDataPersistanceEvent.dispatch( new ModelDataPersistanceEvent( PersistanceEvent.LOAD_FAILED, $mmpe.guid ) );
		}		
	}
	/*
	...
	static public function save( $dbo:DatabaseObject ):void {
		...
		function saved():void 
		{ 
			Log.out("PersistModelData.saveModel.saved - saving changes to guid: " + $dbo.key );
		}	
		
		function failed( $e:PlayerIOError ):void 
		{ 
			Log.out( "PersistModelData.saveModel.failed - error saving changes to guid: " + $dbo.key + " error: " + $e.message, Log.ERROR, $e )
	...
	static public function create( $table:String, $guid:String, $metadata:Object ):void {
		...
		{ 
			if ( dbo ) {
				...
				if ( vmm ) {
					Log.out( "PersistModelData.createModel.created: " + dbo.key ); 
//						MetadataManager.dispatch( new ModelPersistanceEvent( ModelPersistanceEvent.MODEL_CREATE_SUCCEED, $guid, dbo ) );
				}
				else {
					Log.out( "PersistModelData.createModel.created Failed to find voxelmodelmetadata: " + $guid, Log.ERROR ); 
//						MetadataManager.dispatch( new ModelPersistanceEvent( ModelPersistanceEvent.MODEL_CREATE_FAILED, $guid, dbo ) );
				}
			}
			else {
				// This should never happen
				Log.out( "PersistModelData.createModel.created ERROR Unknown unable to save guid: " + $guid, Log.ERROR ); 
//					MetadataManager.dispatch( new ModelPersistanceEvent( ModelPersistanceEvent.MODEL_CREATE_FAILED, $guid, null ) );
		...
		function failed( $e:PlayerIOError ):void 
		{ 
			Log.out( "PersistModelData.createModel.failed - error saving changes to guid: " + $guid + " error: " + $e.message, Log.ERROR, $e )
//				MetadataManager.dispatch( new ModelPersistanceEvent( ModelPersistanceEvent.MODEL_CREATE_FAILED, $guid, null ) );
			// seems like this MIGHT throw it in an endless loop
		} 
	}
...
/*
	...
	static public function loadModelsMetadata( $userName:String, $startDate:Date ):void {
		...
		function loadModelsError(e:PlayerIOError):void {
			Log.out( "PersistModelData.loadModelsError - e: " + e, Log.ERROR );
		}
	}
/*
	static public function loadModelTemplates( $userName:String, $modifiedDate:Date ):void {
		...
		Persistance.loadRange( DB_TABLE_MODELS
					 ...
					 , 100
					, loadObjectsMetadata
					, function (e:PlayerIOError):void {  Log.out( "PersistModelData.errorHandler - e: " + e ); } );
...
/*
	...
	//static public function deleteModel( $guid:String ):void {
		...
		//Persistance.deleteKeys( DB_TABLE_MODELS
							  ...
							  //, function():void
								//{
									//Log.out("PersistModelData.deleteModel - deleted: " + $guid);
								//}
							  //, function(e:PlayerIOError):void
								//{
									//Log.out("PersistModelData.deleteModel - error deleting: " + $guid + " error data: " + e);
								//}
							  //);
	//}
	*/
